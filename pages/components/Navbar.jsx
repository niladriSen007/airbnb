...
const Navbar = ({showLoginForm, setShowLoginForm}) => {
  return (
    <nav  className="relative flex items-center justify-between h-20 bg-gradient-to-l from-slate-900 to-violet-950 px-6 line-clamp-2">
      <div>
      ...
      <div className="flex items-center gap-6">
          ...
          return (
            <div key={id} className="flex items-center gap-2 cursor-pointer">
              ...
              <div className="flex flex-col justify-center -gap-4">
                ...
                <span className="text-xs text-gray-100">{nm?.desc}</span>
              </div>
            </div>
          );
        })}
        <button
          ...
          className="bg-violet-600 hover:bg-violet-900 hover:font-semibold text-white px-4 py-1 rounded-md transition-all duration-300 ml-10 drop-shadow-xl"
        >
          Login/Sign up
        </button>
      </div>
